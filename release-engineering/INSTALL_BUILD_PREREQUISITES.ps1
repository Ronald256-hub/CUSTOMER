...
function Get-WingetCommand {
    ...
    try {
        ...
        Write-Verbose "WinGet registration attempt failed: $($_.Exception.Message)"
    }

    $winget = Get-Command winget.exe -ErrorAction SilentlyContinue
    if (-not $winget) {
        throw "WinGet is required for automatic installation. Install or update Microsoft App Installer and run this script again."
    }
...
if (-not [Environment]::Is64BitOperatingSystem) {
    throw "Nexus POS Windows releases require 64-bit Windows."
}

if (-not (Test-Administrator)) {
    throw "Run PowerShell as Administrator so missing prerequisites can be installed."
}
...
if (-not $dotnet) {
    throw ".NET installation completed but dotnet.exe is not visible. Reopen Administrator PowerShell and run again."
}

$versionText = (& $dotnet.Source --version 2>&1 | Out-String).Trim()
$parsed = $null
if (-not [version]::TryParse(($versionText -split '-')[0], [ref]$parsed) -or
    $parsed.Major -lt 10) {
    throw ".NET SDK 10 or later was not detected. Found: $versionText"
}

if ($IncludeInnoSetup -and -not (Find-InnoCompiler)) {
    throw "Inno Setup was installed but ISCC.exe was not found. Reopen PowerShell and run again."
}

if ($IncludeSigningTools -and -not (Find-SignTool)) {
    throw "Windows SDK was installed but signtool.exe was not found. Reopen PowerShell and run again."
}
