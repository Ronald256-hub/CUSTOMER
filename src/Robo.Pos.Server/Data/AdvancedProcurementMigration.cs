...
/// migration is additive, repeat-safe, preserves earlier records and makes no
/// destructive changes to operational balances.
/// </summary>
public static class AdvancedProcurementMigration
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        Assembly assembly = typeof(AdvancedProcurementMigration).Assembly;
        string resourceName = assembly
            .GetManifestResourceNames()
            .Single(name => name.EndsWith(
                "013_advanced_procurement_inventory.sql",
                StringComparison.Ordinal));

        await using Stream stream =
            assembly.GetManifestResourceStream(resourceName)
            ?? throw new InvalidOperationException(
                "The advanced procurement migration resource was not found.");
