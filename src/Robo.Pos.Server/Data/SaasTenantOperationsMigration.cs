...
public static class SaasTenantOperationsMigration
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        Assembly assembly = typeof(SaasTenantOperationsMigration).Assembly;
        string resourceName = assembly
            .GetManifestResourceNames()
            .Single(name => name.EndsWith(
                "016_saas_tenant_operations.sql",
                StringComparison.Ordinal));

        await using Stream stream =
            assembly.GetManifestResourceStream(resourceName)
            ?? throw new InvalidOperationException(
                "The SaaS tenant operations migration resource was not found.");
