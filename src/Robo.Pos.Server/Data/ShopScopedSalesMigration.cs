...
public static class ShopScopedSalesMigration
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        Assembly assembly = typeof(ShopScopedSalesMigration).Assembly;
        string resourceName = assembly
            .GetManifestResourceNames()
            .Single(name => name.EndsWith(
                "007_shop_scoped_shifts_receipts_reports.sql",
                StringComparison.Ordinal));

        await using Stream stream =
            assembly.GetManifestResourceStream(resourceName)
            ?? throw new InvalidOperationException(
                "The shop-scoped sales migration resource was not found.");
