...
public static class StockTransferInvariantMigration
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        Assembly assembly = typeof(StockTransferInvariantMigration).Assembly;
        string resourceName = assembly
            .GetManifestResourceNames()
            .Single(name => name.EndsWith(
                "009_stock_transfer_invariants.sql",
                StringComparison.Ordinal));

        await using Stream stream =
            assembly.GetManifestResourceStream(resourceName)
            ?? throw new InvalidOperationException(
                "The stock-transfer invariant migration SQL resource was not found.");
