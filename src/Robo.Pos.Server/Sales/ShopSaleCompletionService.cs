...
public sealed class ShopSaleCompletionService
{
    ...
    public ShopSaleCompletionService(
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        IReadOnlyList<GeneratedSaleDocument> registeredDocuments =
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (result is null)
        {
            throw Conflict(
                "open_shift_required",
                "Open a shift at the active shop before completing a sale.");
        }
        ...
        CancellationToken cancellationToken)
    {
        ...
        SELECT
            ...
            shop.is_head_office,
            CASE
                WHEN TRIM(shop.address) <> '' THEN shop.address
                ELSE business.address
            END,
            CASE
                WHEN TRIM(shop.phone) <> '' THEN shop.phone
                ELSE business.phone
            END,
            CASE
                WHEN TRIM(shop.email) <> '' THEN shop.email
                ELSE business.email
            END,
            shop.currency_code,
        ...
        CancellationToken cancellationToken)
    {
        ...
        int? glassSizeMl = reader.IsDBNull(6)
            ? null
            : reader.GetInt32(6);
        int? unitsPerCrate = reader.IsDBNull(7)
            ? null
            : reader.GetInt32(7);
    ...
    private static async Task<string> NextDocumentNumberAsync(
        ...
        CancellationToken cancellationToken)
    {
        string basePrefix = documentType == "invoice" ? "INV" : "RCT";
        string preferredPrefix =
            $"{basePrefix}-{NormalizeShopCode(context.ShopCode)}";
        ...
        await using (var ensure = connection.CreateCommand())
        {
            ...
            ensure.Parameters.AddWithValue("$prefix", preferredPrefix);
        ...
        await using (var read = connection.CreateCommand())
        {
            ...
            if (!await reader.ReadAsync(cancellationToken))
            {
                throw new SalesException(
                    StatusCodes.Status500InternalServerError,
                    "shop_document_sequence_missing",
                    "The active shop document number sequence is missing.");
            }
        ...
        update.CommandText =
        """
        UPDATE shop_document_sequences
        ...
        if (await update.ExecuteNonQueryAsync(cancellationToken) != 1)
        {
            throw Conflict(
                ...
                "Another sale was completed simultaneously at this shop. Try again.");
        }
