...
(function installApprovalsExceptionRoute() {
  const routeId = "exceptions";
  const routeTitle = "Approvals & exceptions";
  const routeSubtitle = "Prioritised approvals, risks and overdue operational actions";
  ...
  function ensureNavigationButton() {
    ...
    button.innerHTML = `<span class="nav-icon" aria-hidden="true">AX</span><span class="nav-label">${routeTitle}</span>`;

    const intelligence = overview.querySelector('[data-page="intelligence"]');
    const dashboard = overview.querySelector('[data-page="dashboard"]');
    if (intelligence) intelligence.insertAdjacentElement("afterend", button);
    else if (dashboard) dashboard.insertAdjacentElement("afterend", button);
    else overview.appendChild(button);
    updateActiveState();
  }
  ...
  function ensureCommandResult() {
    ...
    const searchable = `${routeTitle} approvals exceptions risk overdue actions control inbox`;
    ...
    button.innerHTML = `<span class="nav-icon" aria-hidden="true">AX</span><span><strong>${routeTitle}</strong><small>${routeSubtitle}</small></span>`;
    host.prepend(button);
  }
  ...
  function activateRoute(replaceHistory = true) {
    ...
    updateActiveState();
    window.NexusApprovalsExceptionCentre?.render();
  }
  ...
  new MutationObserver(() => {
    ...
    if (activeRoute()) {
      ...
      if (page && !page.querySelector(".approvals-exception-workspace") && !window.NexusApprovalsExceptionCentre?.isRendering()) {
        window.NexusApprovalsExceptionCentre?.render();
      }
    }
